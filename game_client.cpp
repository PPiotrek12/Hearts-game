...
Trick play_a_card(shared_ptr<Game_stage_client> game, Trick trick) {
    ...
    for (Card card : game->act_deal.cards)
        if (card.color == trick.cards[0].color)
            cards_to_color.push_back(card);
    ...
    for (Card card : trick.cards)
        if (card.value > max_card_from_played.value)
            max_card_from_played = card;
    ...
    if (max_card_lower.value == -1) {
        ...
        for (Card card : cards_to_color)
            if (card.value < max_card_from_color.value)
                max_card_from_color = card;
